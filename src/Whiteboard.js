...
const Whiteboard = () => {
  ...
  useEffect(() => {
    ...
    const updateCanvasSize = () => {
      ...
      const availableWidth = window.innerWidth - sidebarWidth - chatWidth - padding;
      const availableHeight = window.innerHeight - headerHeight;
      
      // Set canvas dimensions with constraints
      canvas.width = Math.max(300, Math.min(800, availableWidth));
      canvas.height = Math.max(200, Math.min(600, availableHeight));
